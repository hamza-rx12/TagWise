...
const AnnotatorDashboard: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const fetchDashboardData = async () => {
            try {
                setLoading(true);
                // Example fetch for tasks, replace with your real endpoint
                const tasksResponse = await fetch('http://localhost:8080/api/v1/annotator/tasks');
                ...
                const pending = tasksData.filter((task: Task) => task.status === 'pending').length;
