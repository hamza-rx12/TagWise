...
interface DatasetAnnotatorsModalProps {
    dataset: Dataset;
    annotators: Annotator[];
    isLoading: boolean;
    onClose: () => void;
    onRemoveAnnotators: (annotatorIds: number[]) => Promise<void>;
}

const DatasetAnnotatorsModal: React.FC<DatasetAnnotatorsModalProps> = ({
    dataset,
    annotators,
    isLoading,
    onClose,
    ...
    const handleRemoveSelected = async () => {
        ...
        setIsRemoving(true);
        try {
            await onRemoveAnnotators(selectedAnnotators.map(id => parseInt(id)));
            setSelectedAnnotators([]);
        } finally {
            setIsRemoving(false);
        }
    ...
    return (
        <Modal onClose={onClose}>
            <div className="p-8 w-full max-w-4xl bg-gradient-to-br from-white to-gray-50 rounded-2xl">
                <div className="mb-8">
                    <h2 className="text-3xl font-bold bg-gradient-to-r from-teal-600 to-teal-800 bg-clip-text text-transparent mb-3">Dataset Annotators</h2>
                    <p className="text-gray-600 text-lg">Annotators working on {dataset.name}</p>
                </div>
                ...
                <div className="space-y-6">
                    {isLoading ? (
                        ...
                        <div className="text-center py-12">
                            ...
                            <h3 className="mt-4 text-xl font-medium text-gray-900">No annotators assigned</h3>
                            <p className="mt-2 text-gray-500 text-base">
                                This dataset doesn't have any assigned annotators yet.
                            </p>
                        </div>
                    ) : (
                        <>
                    ...
                    <div className="flex justify-end space-x-4 mt-8 pt-6 border-t border-gray-200">
                        ...
                        <button
                            ...
                            disabled={selectedAnnotators.length === 0 || isRemoving}
                            className="px-6 py-2.5 bg-gradient-to-r from-red-500 to-red-600 text-white rounded-lg hover:from-red-600 hover:to-red-700 transition-all duration-200 text-base font-medium shadow-sm hover:shadow disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:shadow-none"
                        >
...
export default DatasetAnnotatorsModal; 